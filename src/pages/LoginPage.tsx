...
export function LoginPage() {
  ...
  async function handleGoogleSuccess(credentialResponse: { credential?: string }) {
    ...
    // Try to fetch existing Student ID from backend
    try {
      ...
      console.error('Error checking for existing Student ID:', e)
    }

    // No existing Student ID, auto-create one
    try {
      const autoRes = await fetch('https://english-bot.ohnedan.workers.dev/api/auth/auto-assign-student-id', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ googleEmail: user.email }),
      })
      ...
      console.error('Error auto-assigning Student ID:', e)
    }

    // Fallback: show form if both checks failed
    setGoogleUser(user)
    setIsProcessing(false)
  }
  ...
  const showStudentForm = googleFailed
          ...
          {!showStudentForm ? (
            ...
            <form className={styles.step2} onSubmit={handleLink}>
              ...
              <input
                ...
                autoFocus
              />
              <button className={styles.linkButton} type="submit" disabled={!studentId.trim() || isProcessing}>
                {isProcessing ? 'Processing...' : 'Start studying'} {!isProcessing && <ChevronRight size={14} strokeWidth={2} />}
              </button>
              <button type="button" className={styles.skipButton} onClick={handleBrowseGuest}>
